...
if (isset($_GET['hapus'])) {
  ...
  $q = mysqli_query($conn, "SELECT file_pdf FROM detail_layanan WHERE id_detail='$id'");
  if ($r = mysqli_fetch_assoc($q)) {
    if (!empty($r['file_pdf'])) {
      $path = "../uploads/detail_layanan/pdf/" . $r['file_pdf'];
      if (file_exists($path)) {
        unlink($path);
      }
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (!empty($_FILES['pdf']['name'])) {
    ...
    $targetDir = "../uploads/detail_layanan/pdf/";
    if (!is_dir($targetDir)) {
      mkdir($targetDir, 0777, true);
    }

    if (!move_uploaded_file($_FILES['pdf']['tmp_name'], $targetDir . $namaFile)) {
      die("Gagal upload PDF. Periksa permission folder.");
    }
  }

  $sql = "INSERT INTO detail_layanan (id_profil, judul_detail, isi_detail, file_pdf)
...
$data = mysqli_query($conn, "
  SELECT d.*, p.judul AS judul_profil, b.nama_bidang
  FROM detail_layanan d
  JOIN profil_bidang p ON d.id_profil = p.id_profil
  JOIN bidang b ON p.id_bidang = b.id_bidang
  ORDER BY b.urutan ASC, d.id_detail DESC
");
?>
<!DOCTYPE html>
<html lang="id">
<head>
  ...
  <style>
    ...
    table a { color:#0b5d2a; text-decoration:none; font-weight:500; }
    table a:hover { color:#d4af37; }
  </style>
</head>
<body>
  ...
  <table class="admin-table">
    ...
    <?php while ($d = mysqli_fetch_assoc($data)): ?>
      <tr style="border-top:1px solid #eee;">
        ...
        <td>
          <?php if ($d['file_pdf']): ?>
            <a href="../uploads/detail_layanan/pdf/<?php echo $d['file_pdf']; ?>" target="_blank">Lihat</a>
          <?php else: ?>
            -
          <?php endif; ?>
        </td>
        <td>
          <a href="?hapus=<?php echo $d['id_detail']; ?>"
