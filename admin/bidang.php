...
if (isset($_GET['hapus'])) {
  ...
  // hapus profil bidang terkait
  mysqli_query($conn, "DELETE FROM profil_bidang WHERE id_bidang='$id'");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  // otomatis buat profil bidang
  mysqli_query($conn, "INSERT INTO profil_bidang (id_bidang, judul, deskripsi, punya_detail)
                        VALUES (
                          '$idBaru',
                          '$nama',
                          'Informasi resmi mengenai $nama di Kementerian Agama Kota Semarang.',
                          1
                        )");
} else {
    $id = $_POST['id'];
